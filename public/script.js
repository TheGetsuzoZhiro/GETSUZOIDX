...
function renderTechnicalSignalDetail(s, container) {
  ...
  const entry = s.entryPrice || 0;
  const sl = s.sl || 0;
  const tp1Low = s.target1Low || s.tp1 || 0;
  const tp1High = s.target1High || 0;
  const tp2Low = s.target2Low || s.tp2 || 0;
  const tp2High = s.target2High || 0;

  let takeProfitDisplay = "—";
  let takeProfitPercent = 0;
  let takeProfitColor = "#10b981";
  let takeProfitIcon =
    '<i class="fa-solid fa-bullseye" style="font-size:0.7rem; color:#10b981;"></i>';
  let tpLabel = "TAKE PROFIT";

  if (isClosed && s.exitPrice) {
    takeProfitDisplay = fmtPrice(s.exitPrice);
    const ret = s.returnPercent || 0;
    takeProfitPercent = ret;
    takeProfitColor = ret >= 0 ? "#10b981" : "#ef4444";
    if (ret > 0.01) {
      takeProfitIcon =
    ...
    if (tp2High > 0 && currentPrice >= tp2High) {
      currentTp = tp2High;
      tpLabelText = "TP 2 HIGH";
    } else if (tp2Low > 0 && currentPrice >= tp2Low) {
      currentTp = tp2Low;
      tpLabelText = "TP 2 LOW";
    } else if (tp1High > 0 && currentPrice >= tp1High) {
      currentTp = tp1High;
      tpLabelText = "TP 1 HIGH";
    } else if (tp1Low > 0 && currentPrice >= tp1Low) {
      currentTp = tp1Low;
      tpLabelText = "TP 1 LOW";
    } else {
      currentTp = tp1Low;
      tpLabelText = "TP 1";
    }
    ...
    if (currentTp > 0) {
      takeProfitDisplay = fmtPrice(currentTp);
      if (entry > 0) {
        takeProfitPercent = ((currentTp - entry) / entry) * 100;
      }
      takeProfitColor = takeProfitPercent >= 0 ? "#10b981" : "#ef4444";
      takeProfitIcon =
        '<i class="fa-solid fa-bullseye" style="font-size:0.7rem; color:#10b981;"></i>';
      if (takeProfitPercent > 0.01) {
        takeProfitIcon =
          '<i class="fa-solid fa-arrow-trend-up" style="font-size:0.7rem; color:#10b981;"></i>';
      } else if (takeProfitPercent < -0.01) {
        takeProfitIcon =
          '<i class="fa-solid fa-arrow-trend-down" style="font-size:0.7rem; color:#ef4444;"></i>';
      }
    }
    ...
    if (tp1Low > 0) {
      takeProfitDisplay = fmtPrice(tp1Low);
      if (entry > 0) {
        takeProfitPercent = ((tp1Low - entry) / entry) * 100;
      }
      takeProfitColor = takeProfitPercent >= 0 ? "#10b981" : "#ef4444";
      takeProfitIcon =
        '<i class="fa-solid fa-bullseye" style="font-size:0.7rem; color:#10b981;"></i>';
  ...
  const tpPercentStr =
    takeProfitPercent !== 0
      ? (takeProfitPercent > 0 ? "+" : "") + takeProfitPercent.toFixed(2) + "%"
      : "0.00%";
    ...
    <div style="padding:0.5rem 0.75rem; border-bottom:1px solid rgba(255,255,255,0.06);">
      <div class="price-ladder" style="display:flex; justify-content:space-around; align-items:center; gap:0.5rem; padding:0.2rem 0; margin:0; flex-wrap:wrap;">
        ...
        <div class="price-item" style="display:flex; flex-direction:column; align-items:center; gap:0.2rem; flex:1; min-width:70px; padding:0.3rem; background:rgba(0,0,0,0.15); border-radius:8px;">
          ...
          <span class="value" style="font-family:'JetBrains Mono'; font-weight:600; font-size:0.85rem; color:var(--text-primary);">${entry ? fmtPrice(entry) : "—"}</span>
        ...
        <div class="price-item" style="display:flex; flex-direction:column; align-items:center; gap:0.2rem; flex:1; min-width:70px; padding:0.3rem; background:rgba(0,0,0,0.15); border-radius:8px;">
          <span class="label" style="font-size:0.55rem; color:var(--text-secondary); display:flex; align-items:center; gap:0.2rem;">
            <i class="fa-solid fa-bullseye" style="font-size:0.7rem; color:#10b981;"></i> TAKE PROFIT
          </span>
          <span class="value" style="font-family:'JetBrains Mono'; font-weight:600; font-size:0.85rem; color:${takeProfitColor};">${takeProfitDisplay}</span>
          <span class="change" style="font-size:0.55rem; color:${takeProfitColor};">${tpPercentStr}</span>
        </div>
        ...
        <div class="price-item" style="display:flex; flex-direction:column; align-items:center; gap:0.2rem; flex:1; min-width:70px; padding:0.3rem; background:rgba(0,0,0,0.15); border-radius:8px;">
          ...
          <span class="value" style="font-family:'JetBrains Mono'; font-weight:600; font-size:0.85rem; color:#ef4444;">${sl ? fmtPrice(sl) : "—"}</span>
      ...
      <div style="background:rgba(255,255,255,0.02); border-radius:10px; border:1px solid rgba(255,255,255,0.08); overflow:hidden;">
        ...
        <div style="padding:0.5rem 0.75rem; border-bottom:1px solid rgba(255,255,255,0.06);">
          <div style="display:grid; grid-template-columns: 1fr auto; gap:0.2rem 0.5rem; align-items:center;">
            ...
            <!-- TAGS: Technical + Setup Text (dua baris terpisah) -->
