...
function aggregateSignals(signals) {
  ...
  const waitingSignals = signals.filter(
    (s) => s.status === "WAITING_ENTRY",
  );
  ...
  // totalSignals mencakup semua status (closed + running + waiting)
  result.totalSignals = closed.length + runningSignals.length + waitingSignals.length;
  result.running = runningSignals.length;
  result.waiting = waitingSignals.length;
...
async function showDailySignalDetail(stockCode, signalDate) {
  ...
  if (detailContainer) {
    ...
    if (signal.signalType === "TECHNICAL") {
      ...
      // Pasang tombol back untuk TECHNICAL
...
function renderSignalRows(signals, priceMap, infoMap) {
  ...
  signals.forEach((s) => {
    ...
    // ====== PERUBAHAN UTAMA: tambahkan kondisi untuk TECHNICAL ======
    if (signalType === "BSJP") {
      badgeColor = "#8b5cf6";
