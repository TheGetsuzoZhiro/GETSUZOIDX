// ============================================================
// FULL FIXED CODE - Semua bug diperbaiki
// BSJP: SL = 2%, TP = 3%, Trailing Stop = 3%
// ============================================================
...
async function fetchStockPrice(symbol) {
  ...
  try {
    ...
    const res = await fetch(`/api/price/${symbol}`, { signal: controller.signal });
    clearTimeout(timeoutId);
    if (!res.ok) throw new Error("Network error");
    const data = await res.json();
    const price = data.price ?? null;
    if (price != null) localPrices.set(symbol, price);
    return price;
  } catch (e) {
    console.warn(`fetchStockPrice ${symbol} error:`, e);
    return null;
  }
}
...
async function fetchStockInfo(symbol) {
  ...
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000);
    const res = await fetch(`/api/stock-info/${symbol}`, { signal: controller.signal });
    clearTimeout(timeoutId);
    if (!res.ok) throw new Error("Network error");
    const data = await res.json();
    const safeData = {
      symbol: data.symbol ?? symbol,
      longName: data.longName ?? symbol,
      logoUrl: data.logoUrl ?? null,
    };
    infoCache.set(symbol, { data: safeData, timestamp: Date.now() });
    return safeData;
  } catch (e) {
    console.warn(`fetchStockInfo ${symbol} error:`, e);
...
async function fetchSignals(showLoadingIndicator = true) {
  ...
  const currentTabSnapshot = currentTab;

  if (showLoadingIndicator) {
    if (currentTabSnapshot === 'signals') showLoading('signals');
    if (currentTabSnapshot === 'technical-signals') showLoading('technical-signals');
    if (currentTabSnapshot === 'daily') showLoading('daily');
  }
  ...
  try {
    ...
    } else if (currentTabSnapshot === 'technical-signals') {
      if (isDetailView) {
        isDetailView = false;
      }
    ...
    } else if (currentTabSnapshot === 'daily') {
      if (dailyRendered) {
        await updateDailyContent();
      } else {
        await renderDaily();
      }
    } else if (currentTabSnapshot === 'home') {
    ...
    const containers = {
      ...
      'technical-signals': document.getElementById('technical-signals'),
      'daily': document.getElementById('daily'),
    };
    const container = containers[currentTabSnapshot];
    if (container) {
      container.innerHTML = `
        <div class="loading-state" style="text-align:center; padding:2rem;">
          <div style="display:flex; flex-direction:column; align-items:center; gap:1rem;">
            ...
            <p style="color:#ef4444; font-weight:500; margin:0;">Gagal memuat data</p>
            <button onclick="fetchSignals(true)" class="retry-btn" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);padding:0.6rem 1.2rem;border-radius:8px;color:var(--text-primary);cursor:pointer;display:flex;align-items:center;gap:0.5rem;transition:0.2s;">
              <i class="fa-solid fa-rotate-right"></i> Coba Lagi
            </button>
          </div>
        </div>
      `;
      if (currentTabSnapshot === 'signals') signalListRendered = false;
      else if (currentTabSnapshot === 'technical-signals') technicalListRendered = false;
      else if (currentTabSnapshot === 'daily') dailyRendered = false;
    }
  } finally {
...
async function showSignalList() {
  ...
  const [priceResults, infoResults] = await Promise.allSettled([
    Promise.all(symbols.map(sym => fetchStockPrice(sym))),
    Promise.all(symbols.map(sym => fetchStockInfo(sym))),
  ]);

  const prices = priceResults.status === 'fulfilled' ? priceResults.value : symbols.map(() => null);
  const infos = infoResults.status === 'fulfilled' ? infoResults.value : symbols.map(() => ({ longName: '' }));

  const priceMap = {};
  const infoMap = {};
  symbols.forEach((sym, idx) => {
    priceMap[sym] = prices[idx] ?? null;
    infoMap[sym] = infos[idx] ?? { longName: sym };
  });

  let html = `<div class="sig-list">${renderSignalRows(filteredSignals, priceMap, infoMap)}</div>`;
...
async function showTechnicalSignalList() {
  ...
  let html = `<div class="sig-list">${renderTechnicalRows(techSignals, priceMap, infoMap)}</div>`;
  container.innerHTML = html;
  ...
  container.querySelectorAll('.sig-list-row').forEach(row => {
    row.addEventListener('click', function () {
      const stock = this.dataset.stock;
      const date = this.dataset.date;
      const matchSig = allSignals.find(s => s.stockCode === stock && s.signalDate === date);
      if (matchSig) {
        isDetailView = true;
...
function renderSignalRows(signals, priceMap, infoMap) {
  let rows = "";
  signals.forEach((s) => {
    const stockCode = s.stockCode ?? 'UNKNOWN';
    const signalDate = s.signalDate ?? '';
    const status = s.status ?? 'UNKNOWN';
    const entryPrice = s.entryPrice ?? 0;
    const exitPrice = s.exitPrice ?? null;
    const tp1 = s.tp1 ?? 0;
    const sl = s.sl ?? 0;
    const returnPercent = s.returnPercent ?? 0;
    const confidenceScore = s.confidenceScore ?? 0;
    const signalType = (s.signalType ?? 'WATCHLIST').toUpperCase();
    const confidenceDetails = s.confidenceDetails ?? [];
    const breakEven = s.breakEven ?? false;
    ...
    if (status === "TP") {
      const exit = exitPrice ?? tp1;
      const entry = entryPrice;
      const ret = (entry && exit) ? ((exit - entry) / entry) * 100 : 0;
      const priceVal = exit != null ? fmtPriceNoRp(exit) : "—";
      const sign = ret >= 0 ? "+" : "";
    ...
    } else if (status === "SL" || status === "STOP LOSS") {
      const exit = exitPrice ?? sl;
      const entry = entryPrice;
      const ret = (entry && exit) ? ((exit - entry) / entry) * 100 : 0;
      const priceVal = exit != null ? fmtPriceNoRp(exit) : "—";
      const sign = ret >= 0 ? "+" : "";
      ...
      const currentPrice = priceMap[stockCode];
      const priceVal = currentPrice != null ? fmtPriceNoRp(currentPrice) : "—";
      priceDisplay = priceVal;
      const isRunning = (status === "RUNNING" || status === "TRAILING") && entryPrice && currentPrice;
      if (isRunning) {
        const gainAbs = currentPrice - entryPrice;
        const gainPct = (gainAbs / entryPrice) * 100;
    ...
    const info = infoMap[stockCode] || { longName: stockCode };
    const stockbitUrl = `https://assets.stockbit.com/logos/companies/${stockCode}.png`;
    const parqetUrl = `https://assets.parqet.com/logos/symbol/${stockCode}.png`;
    const bgColor = getColorFromCode(stockCode);
    const logoHtml = `
      <div class="stock-logo-wrapper">
        <img src="${stockbitUrl}" alt="${stockCode}" class="stock-logo"
        ...
        <div class="stock-logo-fallback" style="display:none; background:${bgColor};">${stockCode.substring(0, 2)}</div>
      </div>
    `;

    const confidence = confidenceScore;
    const hasDetails = confidenceDetails.length > 0;
    const isNoData = confidence === 0 && !hasDetails;
    let confDisplay = isNoData ? "" : `<span class="conf-score-badge" data-score="${confidence >= 8 ? 'high' : confidence >= 5 ? 'normal' : confidence >= 3 ? 'low' : 'skip'}">${confidence}/10</span>`;
    ...
    const typeBadge = `<span class="sig-type-badge" style="font-size:0.55rem; font-weight:600; color:${badgeColor}; background:${badgeBg}; padding:0.15rem 0.5rem; border-radius:12px; border:1px solid ${badgeColor}33; display:inline-flex; align-items:center; gap:0.2rem; white-space:nowrap; margin-left:0.3rem;">${badgeIcon.trim().startsWith("<svg") ? badgeIcon : `<i class="fa-solid ${badgeIcon}" style="font-size:0.5rem;"></i>`} ${signalType.replace("STRONG ", "S.")}</span>`;

    rows += `<div class="sig-list-row" data-stock="${stockCode}" data-date="${signalDate}">
      ${logoHtml}
      <div class="sig-list-name">
        <div class="sig-name-row">
          <div class="sig-stock-info">
            <div class="sig-stock-top">
              <span class="sig-stock-code">${escapeHtml(stockCode)}</span>
              ${typeBadge}
              ${confDisplay}
            </div>
...
function renderTechnicalRows(signals, priceMap, infoMap) {
  ...
  signals.forEach((s) => {
    ...
    const techBadge = `<span class="sig-type-badge" style="font-size:0.55rem; font-weight:600; color:#06b6d4; background:rgba(6,182,212,0.15); padding:0.15rem 0.5rem; border-radius:12px; border:1px solid rgba(6,182,212,0.3); display:inline-flex; align-items:center; gap:0.2rem; white-space:nowrap; margin-left:0.3rem;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="width:0.55rem; height:0.55rem; display:block;"><line x1="5" y1="16" x2="5" y2="20" /><line x1="10" y1="11" x2="10" y2="20" /><line x1="15" y1="14" x2="15" y2="20" /><line x1="20" y1="12" x2="20" y2="20" /><path d="M 4 13 L 10 6 L 15 10 L 21 4" /></svg> TECHNICAL</span>`;
...
async function updateSignalList() {
  ...
  try {
    ...
    const symbols = [...new Set(filteredSignals.map(s => s.stockCode))];
    const priceResults = await Promise.allSettled(symbols.map(sym => fetchStockPrice(sym)));
    const priceMap = {};
    symbols.forEach((sym, idx) => {
      priceMap[sym] = priceResults[idx]?.value ?? null;
    });
    ...
    console.warn('updateSignalList error:', err);
  }
}
...
async function updateTechnicalSignalList() {
  ...
  try {
    ...
    console.warn('updateTechnicalSignalList error:', err);
  }
}
...
async function renderDaily() {
  ...
  try {
    ...
    // Event listener untuk filter dll (sama seperti sebelumnya)
    // ... (kode event listener tidak diubah, hanya pastikan pakai try-catch)
    // Kita akan tambahkan event listener dengan try-catch
    ...
    if (summaryContainer) {
      summaryContainer.addEventListener("click", function (e) {
        try {
          ...
          console.warn('TradeSummary click error:', err);
        }
      });
    }
    ...
    if (listToggle && listBody && chevron) {
      listToggle.addEventListener("click", function (e) {
        try {
          ...
          console.warn('List toggle error:', err);
        }
      });
    }
    ...
    c.querySelectorAll(".perf-filter-btn").forEach((btn) => {
      btn.addEventListener("click", function (e) {
        try {
          ...
          console.warn('Filter button error:', err);
        }
      });
    });
    ...
    c.innerHTML = `
      <div class="loading-state" style="text-align:center; padding:2rem;">
        <p style="color:#ef4444;">Gagal memuat laporan harian</p>
        <button onclick="renderDaily()" class="retry-btn">Coba Lagi</button>
      </div>
    `;
    dailyRendered = false;
  }
...
async function renderPerformanceSignalList(status) {
  ...
  try {
    ...
    const infoMap = {};
    symbols.forEach((sym, idx) => {
      priceMap[sym] = priceResults.status === 'fulfilled' ? priceResults.value[idx] : null;
      infoMap[sym] = infoResults.status === 'fulfilled' ? infoResults.value[idx] : { longName: sym };
    });
    ...
    container.querySelectorAll('.sig-list-row').forEach((row) => {
      row.addEventListener('click', function () {
        const stock = this.dataset.stock;
        const date = this.dataset.date;
        if (stock && date) showDailySignalDetail(stock, date);
      });
    });
  } catch (err) {
    console.error("renderPerformanceSignalList error:", err);
...
// ============================================================
// RENDER STRATEGY FLOW FOR SIGNAL (dengan fallback)
// ============================================================
function renderStrategyFlowForSignal(s) {
  if (!s) return '<div>Tidak ada data strategi</div>';
  ...
  if (entry > 0 && tp > 0) tpPercent = ((tp - entry) / entry) * 100;

  // BSJP: SL = 2%, TP = 3%, Trailing = 3%
  const isBsjp = signalType === 'BSJP';
  let slLabel, tpLabel, trailingLabel = 'TS 5%';
  if (isBsjp) {
    slLabel = '-2%';
    tpLabel = '+3%';
    trailingLabel = 'TS 3%';
  } else {
    const slLabelRaw = slPercent < 0 ? `${slPercent.toFixed(1)}%` : `-${slPercent.toFixed(1)}%`;
    const tpLabelRaw = tpPercent > 0 ? `+${tpPercent.toFixed(1)}%` : `${tpPercent.toFixed(1)}%`;
    slLabel = slLabelRaw;
    tpLabel = tpLabelRaw;
  }
  ...
  if (status === 'SL' && !breakEven) step1State = 'failed';

  const step2Active = breakEven === true || status === 'TRAILING' || status === 'TP';
  const step2State = status === 'SL' && breakEven ? 'warning' : status === 'TP' ? 'success' : 'default';

  const step3Active = status === 'TRAILING' || status === 'TP';
  let step3State = 'default';
  if (status === 'SL' && breakEven) step3State = 'warning';
  else if (status === 'TP') step3State = 'success';
  ...
  return `
    <div style="background:rgba(255,255,255,0.01); border:1px solid rgba(255,255,255,0.08); border-radius:8px; padding:0.65rem 0.75rem; margin-top:0.5rem;">
      <div style="display:flex; align-items:center; gap:0.4rem; margin-bottom:0.1rem;">
        ...
        ${status === 'RUNNING' ? `<span style="font-size:0.55rem; background:rgba(16,185,129,0.15); color:#10b981; padding:0.1rem 0.5rem; border-radius:12px; margin-left:auto;">Active</span>` : 
          status === 'TRAILING' ? `<span style="font-size:0.55rem; background:rgba(245,158,11,0.15); color:#f59e0b; padding:0.1rem 0.5rem; border-radius:12px; margin-left:auto;">Trailing</span>` :
          status === 'WAITING_ENTRY' ? `<span style="font-size:0.55rem; background:rgba(59,130,246,0.15); color:#3b82f6; padding:0.1rem 0.5rem; border-radius:12px; margin-left:auto;">Waiting</span>` :
          `<span style="font-size:0.55rem; background:rgba(255,255,255,0.05); color:var(--text-secondary); padding:0.1rem 0.5rem; border-radius:12px; margin-left:auto;">${status}</span>`
        }
      </div>
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; margin:0.8rem 0; position:relative; padding:0 0.5rem;">
        ...
        ${stepCircle(step3Active, "Trailing Stop", `${trailingLabel}`, "3", step3State)}
      </div>
...
// ============================================================
// RENDER BSJP DETAIL (dengan SL=2%, TP=3%, Trailing=3%)
// ============================================================
function renderBsjpDetail(s, container, onBack) {
  if (!s || !container) {
    container.innerHTML = `<div style="color:#ef4444;">Data tidak valid</div>`;
    return;
  }

  let stockInfo = { longName: s.stockCode ?? 'Unknown', logoUrl: null };
  let currentPrice = null;

  Promise.allSettled([
    fetchStockInfo(s.stockCode).catch(() => null),
    fetchStockPrice(s.stockCode).catch(() => null),
  ]).then(([infoResult, priceResult]) => {
    if (infoResult.status === 'fulfilled' && infoResult.value) stockInfo = infoResult.value;
    if (priceResult.status === 'fulfilled') currentPrice = priceResult.value;
    renderBsjpDetailContent(s, container, onBack, currentPrice, stockInfo);
  }).catch(err => {
    console.warn('renderBsjpDetail error:', err);
...
function renderBsjpDetailContent(s, container, onBack, currentPrice, stockInfo) {
  // Definisikan SL=2%, TP=3%, Trailing=3%
  const SL_PERCENT = 0.02;  // 2%
  const TP_PERCENT = 0.03;  // 3%
  const TRAILING_PERCENT = 0.03; // 3%
  ...
  // Strategy Visual untuk BSJP (SL=2%, TP=3%, Trailing=3%)
  const strategyVisual = `
    <div style="display:flex; align-items:center; justify-content:space-between; margin:0.8rem 0; position:relative; padding:0 0.5rem;">
      ...
      ${stepCircle(isStep3Active, "Trailing Stop", "Trailing 3%", "3", step3State)}
    </div>
    <div style="display:flex; justify-content:center; gap:0.5rem; font-size:0.55rem; color:var(--text-secondary); margin-top:0.2rem;">
      <span style="display:flex; align-items:center; gap:0.2rem;">
        <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#10b981;"></span> Active
  ...
  bsjpRefreshInterval = setInterval(async () => {
    try {
      ...
      const updated = all.find(
        (sig) => sig.stockCode === s.stockCode && sig.signalDate === s.signalDate,
      );
      if (updated) {
        const changed =
...
// ============================================================
// RENDER SIGNAL DETAIL TO CONTAINER (umum)
// ============================================================
async function renderSignalDetailToContainer(signal, container, onBack) {
  ...
  try {
    ...
    } else if ((s.status === 'TP' || s.status === 'SL') && !currentPrice) {
    ...
    if (s.confidenceDetails && s.confidenceDetails.length > 0) {
      ...
      s.confidenceDetails.forEach((d) => {
        ...
        const icon = isPos ? `<i class="fa-solid fa-circle-check" style="color:#10b981; font-size:14px;"></i>` : `<i class="fa-solid fa-circle-xmark" style="color:#ef4444; font-size:14px;"></i>`;
    ...
    // Strategy Flow
    const strategyFlowHtml = renderStrategyFlowForSignal(s);

    // Price ladder
    const entry = s.entryPrice || 0;
    const tp = s.tp1 || entry * 1.1;
    const sl = s.sl || entry * 0.9;
    const pctTp = s.tp1 && s.entryPrice ? (((s.tp1 - s.entryPrice) / s.entryPrice) * 100).toFixed(2) : '–';
    const pctSl = s.sl && s.entryPrice ? (((s.sl - s.entryPrice) / s.entryPrice) * 100).toFixed(2) : '–';
    ...
    <div style="padding:0.5rem 0.75rem; border-bottom:1px solid rgba(255,255,255,0.06);">
      <div class="pro-grid-2" style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem;">
        ...
        <div>
          ...
          <div class="foreign-flow-container"><div class="foreign-row"><span class="foreign-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px;"><circle cx="12" cy="12" r="10"/><path d="M12 8v8M8 12h8"/></svg> NET BUY ASING</span><span class="foreign-value buy">+123.456 lot</span><div class="foreign-bar-track"><div class="foreign-bar-fill buy-fill" style="width:75%;"></div></div><span class="foreign-participation">25%</span></div><div style="font-size:0.65rem; color:var(--text-secondary); display:flex; justify-content:space-between; margin-top:0.3rem;"><span>Partisipasi</span><span>25%</span></div></div>
        </div>
      </div>
    </div>

    <!-- Teknikal Chart (placeholder) -->
    <div style="padding:0.5rem 0.75rem; border-bottom:1px solid rgba(255,255,255,0.06);">
      <div class="pro-grid-2" style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem;">
        <div>
          <div style="display:flex; align-items:center; gap:0.3rem; margin-bottom:0.3rem; font-weight:600; font-size:0.8rem; color:var(--text-secondary);">
    ...
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        try {
          ...
          console.warn('Detail chart error:', chartErr);
        }
      }, 50);
    });

  } catch (err) {
    console.error('renderSignalDetailToContainer error:', err);
    container.innerHTML = `<div style="color:#ef4444; padding:1rem;">Gagal menampilkan detail</div>`;
  }
}

// ============================================================
// RENDER TECHNICAL SIGNAL DETAIL
// ============================================================
function renderTechnicalSignalDetail(s, container) {
  ...
  try {
    const isExpired = s.status === 'EXPIRED' || s.expired === true ||
                      (s.status === 'CLOSED' && (s.returnPercent === 0 || s.returnPercent === null)) ||
                      (s.status === 'WAITING_ENTRY' && s.signalDate && new Date(s.signalDate) < new Date(Date.now() - 7*24*60*60*1000));
    ...
    const strategyFlow = `
      <div style="background:rgba(255,255,255,0.01); border:1px solid rgba(255,255,255,0.08); border-radius:8px; padding:0.65rem 0.75rem; margin-top:0.5rem;">
        ...
        ${isExpired ? `<div style="text-align:center; margin-top:0.4rem; padding:0.3rem 0.5rem; background:rgba(113,113,122,0.08); border-radius:6px; font-size:0.6rem; color:#71717a; border:1px dashed rgba(113,113,122,0.15);">
          <i class="fa-regular fa-clock" style="margin-right:0.3rem;"></i> Signal telah kedaluwarsa — Tidak ada alur aktif
        </div>` : s.status === 'TRAILING' ? `<div style="text-align:center; margin-top:0.4rem; padding:0.35rem 0.5rem; background:rgba(245,158,11,0.08); border-radius:6px; font-size:0.6rem; color:#f59e0b; border:1px solid rgba(245,158,11,0.25);">
        ...
        </div>` : `<div style="text-align:center; margin-top:0.4rem; font-size:0.55rem; color:var(--text-secondary); opacity:0.4;">
    ...
    if (entry > 0 && dynamicTpVal > 0) dynamicTpPercent = ((dynamicTpVal - entry) / entry) * 100;
    ...
    const strategyDetail = `
      <div style="background:rgba(255,255,255,0.02); border-radius:6px; padding:0.5rem 0.6rem; margin-top:0.5rem; border:1px solid rgba(255,255,255,0.05); display:flex; flex-direction:column; gap:0.35rem; font-size:0.65rem; color:var(--text-secondary); line-height:1.3;">
        ...
        <div style="display:flex; align-items:start;">
          <i class="fa-solid fa-shield-cat" style="color:#ef4444; font-size:0.5rem; margin-right:0.4rem; margin-top:0.15rem;"></i>
          <span>Stop Loss awal <strong>-${s.stopLossPercent || 5}%</strong> dari entry untuk proteksi kerugian.</span>
        </div>
      </div>
    `;
    ...
    console.error('renderTechnicalSignalDetail error:', err);
    container.innerHTML = `
      <div style="color:#ef4444; padding:1rem;">
        <i class="fa-solid fa-triangle-exclamation"></i> Gagal menampilkan detail teknikal
        <button onclick="showTechnicalSignalList()" class="sig-back-btn">Kembali</button>
      </div>
    `;
  }
}
...
async function showSignalDetail(index) {
  ...
  if (!s) {
    console.warn('Signal not found at index', index);
    return;
  }
  ...
  try {
    ...
    console.error('showSignalDetail error:', err);
    container.innerHTML = `<div style="color:#ef4444;padding:1rem;">Gagal menampilkan detail</div>`;
  }
}

// ============================================================
// SHOW SIGNAL LIST (getSortedSignals)
// ============================================================
...
async function updateDailyContent() {
  try {
    ...
    console.warn('updateDailyContent error:', err);
    const container = document.getElementById('dailyContentWrapper');
    if (container) {
      container.innerHTML = `<div style="color:#ef4444; padding:1rem;">Gagal memperbarui konten harian</div>`;
    }
  }
}

// ============================================================
// RENDER BROKER FLOW, PATTERN VISUAL, DETAIL CHARTS (helper)
// ============================================================
function renderBrokerFlow(topBuyers, topSellers, sinyalBandar, container = document) {
  let containerEl = container.querySelector ? container.querySelector("#brokerFlowContainer") : null;
  if (!containerEl) containerEl = document.getElementById("brokerFlowContainer");
  if (!containerEl) return;

  containerEl.innerHTML = "";
  if (!sinyalBandar || sinyalBandar.trim() === "") {
    containerEl.innerHTML = '<div style="text-align:center;padding:0.5rem 0;color:var(--text-secondary);font-size:0.7rem;"><i class="fas fa-database" style="margin-right:0.3rem;"></i> Bandarmology: Data sedang maintenance</div>';
    return;
  }
  ...
  if (normalized.includes("STRONG BUY")) { bandarSignal = "STRONG BUY"; bandarClass = "strong-buy"; }
  else if (normalized.includes("BUY") && !normalized.includes("STRONG")) { bandarSignal = "BUY"; bandarClass = "buy"; }
  else if (normalized.includes("STRONG SELL")) { bandarSignal = "STRONG SELL"; bandarClass = "strong-sell"; }
  else if (normalized.includes("SELL") && !normalized.includes("STRONG")) { bandarSignal = "SELL"; bandarClass = "sell"; }
  else if (normalized.includes("NEUTRAL")) { bandarSignal = "NEUTRAL"; bandarClass = "neutral"; }
  ...
  if (topBuy.length) {
    topBuy.forEach((b) => {
      ...
      html += `<div class="broker-item buy"><span class="code">${b.code}</span><span class="vol">${b.lot.toLocaleString()}L</span><div style="flex:1;height:3px;background:rgba(255,255,255,0.05);border-radius:2px;overflow:hidden;"><div style="width:${Math.min(pct, 100)}%;height:100%;background:var(--success);border-radius:2px;"></div></div></div>`;
    });
  } else {
  ...
  html += `<div class="broker-center"><span class="broker-flow-arrow">⟷</span><span class="broker-signal ${bandarClass}">${bandarSignal}</span><span style="font-size:0.6rem;color:var(--text-secondary);">Smart Money</span></div>`;

  html += `<div class="broker-col"><div class="broker-col-title sell">▼ SELL</div>`;
  if (topSell.length) {
    topSell.forEach((b) => {
      const pct = (b.lot / maxSell) * 100;
      html += `<div class="broker-item sell"><div style="flex:1;height:3px;background:rgba(255,255,255,0.05);border-radius:2px;overflow:hidden;"><div style="width:${Math.min(pct, 100)}%;height:100%;background:var(--danger);border-radius:2px;margin-left:auto;"></div></div><span class="vol">${b.lot.toLocaleString()}L</span><span class="code">${b.code}</span></div>`;
    });
  } else {
...
function renderPatternVisual(patternText, container = document) {
  const containerEl = container.getElementById("patternVisualContainer") || container.querySelector("#patternVisualContainer");
  if (!containerEl) return;
  const patterns = {
    "Recovery Uptrend dari Bottom": { path: "M10,70 Q40,80 60,40 Q80,10 100,20", color: "#10b981", label: "Recovery" },
    "Breakout Bollinger Upper": { path: "M10,60 Q30,50 50,55 Q70,60 90,20 L100,10", color: "#f59e0b", label: "Breakout" },
    "Bullish Momentum Candle": { path: "M10,80 Q30,70 50,50 Q70,30 90,20", color: "#3b82f6", label: "Momentum" },
    "Strong Close Near High": { path: "M10,70 Q30,50 50,30 Q70,20 90,10 L100,10", color: "#8b5cf6", label: "Strong" },
  };
  let found = null;
  if (patternText) {
  ...
  if (!found) found = { path: "M10,70 Q40,80 60,40 Q80,10 100,20", color: "#71717a", label: "Pattern" };
...
// ============================================================
// UPDATE TOTAL SIGNALS & CHECK CHANGES
// ============================================================
...
function checkSignalChanges(running, closed) {
  ...
  const currentRunningIds = running.map(s => `${s.stockCode}-${s.signalDate}`).sort().join(",");
  const currentClosedIds = closed.map(s => `${s.stockCode}-${s.signalDate}`).sort().join(",");
  ...
  if (newRunning.length > 0) {
    const newSignals = running.filter(s => newRunning.includes(`${s.stockCode}-${s.signalDate}`));
    ...
    function handleGroupNotification(groupName, signals) {
    ...
    handleGroupNotification("SESI 1", groups.session1);
    handleGroupNotification("SESI 2", groups.session2);
    handleGroupNotification("BSJP", groups.bsjp);
    handleGroupNotification("LAINNYA", groups.other);
  }

  const prevClosedArr = prevClosedIds ? prevClosedIds.split(",") : [];
  const currentClosedArr = currentClosedIds ? currentClosedIds.split(",") : [];
  const newClosed = currentClosedArr.filter(id => !prevClosedArr.includes(id));
  if (newClosed.length > 0) {
    const closedSignals = closed.filter(s => newClosed.includes(`${s.stockCode}-${s.signalDate}`));
    closedSignals.forEach((s) => {
      const ret = s.returnPercent || 0;
      const sign = ret >= 0 ? "+" : "";
      const emoji = s.status === "TP" ? "✅" : "❌";
      addNotification("Signal Closed", `${emoji} ${s.stockCode} Selesai ${sign}${ret.toFixed(2)}%`, "closed");
    });
  }
...
function updateEquityChart(data) {
  ...
  const closed = data.closed || [];
  let labels = ["Start"], equityData = [100];
  if (closed.length) {
    let current = 100;
    closed.forEach((t, i) => {
      labels.push(`T${i + 1}`);
...
function updateWinRateChart(data) {
  ...
  let totalRisk = 0, totalReward = 0, grossProfit = 0, grossLoss = 0;
  closed.forEach((s) => {
    const ret = s.returnPercent || 0;
    if (ret > 0) { grossProfit += ret; totalReward += ret; totalRisk += ret * 0.5; }
    else { grossLoss += Math.abs(ret); totalRisk += Math.abs(ret); }
  });
  ...
  winRateChart = new Chart(ctx, {
    ...
    data: {
      labels: ["Win", "Loss"],
      datasets: [{ data: dataChart, backgroundColor: colors, borderColor: "transparent", cutout: "85%" }],
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
...
function renderNotificationModal() {
  ...
  modalContent.style.cssText = `
    ...
    border-radius: 16px; max-width: 500px; width: 100%; max-height: 80vh;
  ...
  body.style.cssText = `padding: 0.5rem 1.25rem 1.25rem; overflow-y: auto; flex: 1;`;

  if (notificationHistory.length === 0) {
    body.innerHTML = `<div style="text-align:center; color:var(--text-secondary); opacity:0.4; padding:2rem 0;"><i class="fas fa-bell-slash" style="font-size:2rem; display:block; margin-bottom:0.5rem;"></i>No notifications yet</div>`;
  } else {
    let listHtml = "";
...
function updatePriceElement(symbol, price) {
  ...
  runningSignals.forEach((signal) => {
    const rows = document.querySelectorAll(`.sig-list-row[data-stock="${symbol}"][data-date="${signal.signalDate}"]`);
    rows.forEach((row) => {
      const priceEl = row.querySelector(".stock-price");
      const gainEl = row.querySelector(".sig-right span:last-child");
      if (!priceEl) return;
      if (price != null) {
        ...
        if (Math.abs(gain) < 0.01) {
          ...
          if (gainEl) { gainEl.innerHTML = `<i class="fa-solid fa-arrow-trend-up" style="font-size:0.7rem; color:#10b981;"></i> +${absGain}%`; gainEl.style.color = "#10b981"; }
          arrow = '<i class="fa-solid fa-arrow-up" style="color:#10b981; font-size:0.7rem; margin-right:0.1rem;"></i>';
        } else {
          const absGain = Math.abs(gain).toFixed(2);
          if (gainEl) { gainEl.innerHTML = `<i class="fa-solid fa-arrow-trend-down" style="font-size:0.7rem; color:#ef4444;"></i> -${absGain}%`; gainEl.style.color = "#ef4444"; }
...
// ============================================================
// FETCH REPORTS, POLLING, CLOCK, ETC
// ============================================================
...
function startPolling() {
  ...
  pollingInterval = setInterval(() => {
    const activeTab = document.querySelector(".view.active")?.id;
    if (activeTab === "home") fetchReports();
...
function updateLastUpdate() {
  ...
  if (el) el.innerText = new Date().toLocaleString("id-ID", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
}

function updateClock() {
  const clockEl = document.getElementById("clockDisplay");
  if (clockEl) clockEl.innerText = new Date().toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
...
function initTabs() {
  ...
  btns.forEach((btn) => {
    ...
    btn.addEventListener("click", function (e) {
      ...
      if (tabId !== "home") homeLoaded = false;
...
function closeAllDropdowns() {
  ...
  if (signalSub) { signalSub.classList.remove("open"); signalSub.style.display = "none"; }
  if (signalParent) {
  ...
  if (techSub) { techSub.classList.remove("open"); techSub.style.display = "none"; }
  if (techParent) {
...
function initNotifications() {
  ...
  if (notifBtn) {
    notifBtn.addEventListener("click", async () => {
      ...
      if (success) alert("✅ Notifikasi aktif! Token baru tersimpan.");
      else alert("❌ Gagal mengaktifkan notifikasi. Pastikan browser mendukung dan izin diberikan.");
    });
  }
}
...
const VAPID_PUBLIC_KEY = "BCGyIOUseFBON2YXTAk-rcvncZ65jkbKqb2ShjOuvZhP08HLvaJJis5Bsx8ybuVVcZbXZow5GRrl9ykSiV0Y3B0";
...
window.addEventListener('unhandledrejection', function(event) {
  ...
  if (spinner) {
    const container = spinner.closest('.view') || document.body;
    container.innerHTML = `
      <div style="text-align:center; padding:2rem; color:#ef4444;">
        <i class="fa-solid fa-triangle-exclamation"></i> Terjadi kesalahan, silakan refresh halaman.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!currentHash.startsWith("#home") && !currentHash.startsWith("#detail-") && !currentHash.startsWith("#technical-") && !currentHash.startsWith("#signals-")) {
    window.location.hash = "home";
  }
